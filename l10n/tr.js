OC.L10N.register(
    "sketch_picker",
    {
    "Draw a sketch" : "Bir taslak çizin",
    "Sketch picker" : "Taslak seçici",
    "Quickly draw and share sketches" : "Taslakları hızlıca çizin ve paylaşın",
    "This app adds a smart picker provider to quickly draw sketches and insert them in your current context." : "Bu uygulama, taslakları hızlıca çizmek ve bunları var olan bağlamınıza eklemek için bir akıllı seçici sağlayıcısı ekler.",
    "Error saving image" : "Görsel kaydedilirken sorun çıktı",
    "Really?" : "Gerçekten mi?",
    "Unsaved changes" : "Kaydedilmemiş değişiklikler",
    "Drop changes" : "Değişiklikleri yok say",
    "Cancel" : "İptal",
    "Loading sketch" : "Taslak yükleniyor",
    "Choose a file to draw a sketch on" : "Üzerine taslağın çizileceği bir dosya seçin",
    "Failed to save image" : "Görsel kaydedilemedi",
    "Unknown Sketch Picker API error" : "Bilinmeyen Sketch Picket API sorunu",
    "Open from Files" : "Dosyalar uygulamasından aç",
    "Open recently seen sketch" : "Son bakılan taslağı aç",
    "Name" : "Ad",
    "Post sketch" : "Taslak gönder",
    "Save as" : "Farklı kaydet",
    "Back" : "Geri",
    "Loading …" : "Yükleniyor…",
    "Reset" : "Sıfırla",
    "All changes will be lost." : "Tüm değişiklikler kaybolacak.",
    "Are you sure you want to continue?" : "İlerlemek istediğinize emin misiniz?",
    "Continue" : "Sürdür",
    "Undo" : "Geri al",
    "Redo" : "Yinele",
    "Show original image" : "Özgün görseli görüntüle",
    "Zoom in" : "Yakınlaştır",
    "Zoom out" : "Uzaklaştır",
    "Toggle zoom menu" : "Yakınlaştırma menüsünü aç/kapat",
    "Adjust" : "Ayarla",
    "Fine-tune" : "İnce ayar",
    "Filters" : "Süzgeçler",
    "Watermark" : "Filigran",
    "Draw" : "Çiz",
    "Resize" : "Yeniden boyutlandır",
    "Invalid image." : "Görsel geçersiz.",
    "Error while uploading the image." : "Görsel yüklenirken sorun çıktı.",
    "are not images" : "görseller değil",
    "is not an image" : "bir görsel değil",
    "to be uploaded" : "yüklenecek",
    "Crop" : "Kırp",
    "Original" : "Özgün",
    "Custom" : "Özel",
    "Square" : "Kare",
    "Landscape" : "Yatay",
    "Portrait" : "Dikey",
    "Ellipse" : "Elips",
    "Classic TV" : "Klasik TV",
    "CinemaScope" : "Sinemaskop",
    "Arrow" : "Ok",
    "Blur" : "Bulanıklaştır",
    "Brightness" : "Parlaklık",
    "Contrast" : "Karşıtlık",
    "Un-flip X" : "X ekseninde ters çevirmeyi geri al",
    "Flip X" : "X ekseninde ters çevir",
    "Un-flip Y" : "Y ekseninde ters çevirmeyi geri al",
    "Flip Y" : "Y ekseninde ters çevir",
    "HSV" : "HSV",
    "Hue" : "Renk tonu",
    "Saturation" : "Doygunluk",
    "Value" : "Değer",
    "Image" : "Görsel",
    "Importing …" : "İçe aktarılıyor…",
    "+ Add image" : "+ Görsel ekle",
    "Line" : "Çizgi",
    "Pen" : "Kalem",
    "Polygon" : "Çokgen",
    "Sides" : "Yanlar",
    "Rectangle" : "Dikdörtgen",
    "Corner Radius" : "Köşe yarıçapı",
    "Width in pixels" : "Piksel cinsinden genişlik",
    "Height in pixels" : "Piksel cinsinden yüksekilk",
    "Toggle ratio lock" : "En boy oranı kilidini aç/kapat",
    "Reset to original image size" : "Özgün görsel boyutuna sıfırla",
    "Rotate" : "Döndür",
    "Text" : "Yazı",
    "Text spacing" : "Yazı aralığı",
    "Text alignment" : "Yazı hizalaması",
    "Font family" : "Yazı tipi ailesi",
    "Size" : "Boyut",
    "Letter spacing" : "Harf aralığı",
    "Line height" : "Satır yüksekliği",
    "Warmth" : "Sıcaklık",
    "+ Add watermark" : "+ Filigran ekle",
    "Choose watermark type" : "Filigran türünü seçin",
    "Upload watermark" : "Filigran yükle",
    "Add as text" : "Yazı olarak ekle",
    "Padding" : "İç kenar boşluğu",
    "Shadow" : "Gölge",
    "Horizontal" : "Yatay",
    "Vertical" : "Dikey",
    "Opacity" : "Matlık",
    "Position" : "Konum",
    "Stroke" : "Kalınlık",
    "Save image as" : "Görseli farklı kaydet",
    "Extension" : "Uzantı",
    "Name is required." : "Adın yazılması zorunludur.",
    "Quality" : "Kalite",
    "Saved image size (width x height)" : "Kaydedilmiş görsel boyutu (genişlik x yükseklik)",
    "Note that the selected crop area is lower than the applied resize which might cause quality decrease" : "Seçilmiş kırpma alanı uygulanan yeniden boyutlandırma alanından küçük. Bu durum görsel kalitesinin düşmesine neden olabilir.",
    "Actual size (100%)" : "Gerçek boyut (%100)",
    "Fit size" : "Boyutu sığdır"
},
"nplurals=2; plural=(n > 1);");
